...
use crate::{UserEvent, image_decode, image_zoom_math::pan_after_zoom_to_focal, viewer_viewport};
...
impl ImageTab {
    pub fn open(
        ...
        let watcher = FileWatcher::new(build_source_notifier(proxy, window_id, id));
        let tab = Self {
            id,
            path: path.clone(),
