...
- LLM idle 且无 WebSocket observer 时，释放 LLM client、stream、tool executor、subscriber 等运行对象，只保留可恢复状态。
...
`AgentInstance` 是后端 workspace 级实体。`ActiveAgentTurn` 只在 LLM 正在输出、tool call 正在执行、或取消正在传播时存在。Agent idle 后必须释放 `ActiveAgentTurn` 内的运行对象。
...
每个 WebSocket connection 注册为 runtime subscriber。Agent 产生事件后先写入 runtime event log，再广播给当前所有 subscriber。
...
- Agent idle 且 subscriber 数为 0 时，释放所有运行对象，仅保留 `StoredAgentState`、chat binding 和 event log。
...
- Agent idle 且无 subscriber 时，不再持有 active LLM client、provider stream、tool executor 或 WebSocket push handle。
