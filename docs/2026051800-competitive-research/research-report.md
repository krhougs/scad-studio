...
| **Web-based model explorer** / Web 端模型浏览器 | CAD Explorer (React + Three.js + Vite, file browser, assembly inspection) | studio-web skeleton exists (72 files: CadQuery viewer, selection, chat, workspace tree, inspector, export) but end-to-end flow not yet validated / studio-web 骨架已有（72 个文件），但端到端流程尚未验证 | **High / 高** |
