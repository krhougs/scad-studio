...
/// Abstract input event type for camera interaction (decoupled from winit).
#[derive(Debug, Clone, Copy)]
...
impl CameraInteraction {
    pub fn handle_event(&mut self, camera: &mut OrbitalCamera, event: &CameraInputEvent) -> bool {
        match event {
            CameraInputEvent::MousePress(button) => self.handle_mouse_press(*button),
            CameraInputEvent::MouseRelease(button) => self.handle_mouse_release(*button),
            CameraInputEvent::CursorMove { x, y } => self.handle_cursor(camera, *x, *y),
            CameraInputEvent::Scroll { delta_y } => self.handle_wheel(camera, *delta_y),
        }
    }
    ...
    fn handle_mouse_release(&mut self, button: MouseButton) -> bool {
        let mode = match button {
            MouseButton::Left => DragMode::Orbit,
            MouseButton::Middle | MouseButton::Right => DragMode::Pan,
            _ => return false,
        };
        if self.drag_mode == Some(mode) {
            self.drag_mode = None;
    ...
    fn handle_cursor(&mut self, camera: &mut OrbitalCamera, x: f32, y: f32) -> bool {
    ...
    fn handle_wheel(&mut self, camera: &mut OrbitalCamera, delta_y: f32) -> bool {
        camera.zoom(delta_y);
        true
    }
}
