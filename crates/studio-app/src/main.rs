...
use scad_ui::{
    ...
    tab_system::TabId,
    theme,
    viewer_event_routing::{self, ViewerEventKind},
};
...
use studio_app::{
    ...
    font_setup,
    image_tab::ImageTab,
    layout::{self, LayoutAction},
    macos_fused_titlebar,
    markdown_tab::MarkdownTab,
    platform_menu::{APP_NAME, MenuCommand, PlatformMenu},
    protocol_client::{DesktopProtocolClient, WatchSubscriptionHandle},
    studio_document::StudioDocumentSession,
    viewer_tab::{ViewerTab, ViewerUiOutcome},
};
use studio_common::{AppConfig, DocumentDescriptor, DocumentKind, sanitize_recent_workspaces};
use winit::{
...
struct StudioRuntime {
    ...
    app: StudioApp,
    workspace_watch: Option<WatchSubscriptionHandle>,
    redraw_queued: bool,
...
fn main() {
    ...
    if args.iter().any(|arg| arg == "--smoke-exit") {
        if let Err(error) = run_smoke_exit() {
            eprintln!("{error}");
            std::process::exit(1);
        }
        return;
    ...
    let client =
        DesktopProtocolClient::connect().expect("desktop protocol client should initialize");
    let config = load_app_config(&client);
...
impl ApplicationHandler<UserEvent> for StudioDesktopApp {
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: UserEvent) {
        match event {
            ...
            UserEvent::PreviewReady(window_id, tab_id, serial, result) => {
                self.handle_preview_ready(window_id, tab_id, serial, result)
            }
...
impl StudioDesktopApp {
    ...
    fn handle_layout_action(
        ...
        match action {
            ...
            LayoutAction::LoadDirectory(path) => self.load_directory(window_id, path),
            LayoutAction::SentChat(message) => {
                if let Some(state) = self.windows.get_mut(&window_id) {
                    state
                        .app
                        .push_log(LogLevel::Info, format!("Chat 输入: {message}"));
                    schedule_redraw(state);
                }
            }
        }
    ...
    fn handle_preview_ready(
        ...
        tab_id: TabId,
        serial: u64,
        result: Result<studio_app::protocol_client::PreviewSuccess, String>,
    ) {
        let Some(state) = self.windows.get_mut(&window_id) else {
            return;
        };
        ...
        {
            tab.handle_preview_ready(serial, result);
            schedule_redraw(state);
        }
    }
    ...
    fn handle_source_change(&mut self, window_id: WindowId, tab_id: TabId, path: PathBuf) {
        ...
        if let Some(session) = state.app.document_by_legacy_tab_id_mut(tab_id) {
            ...
            {
                state.app.push_log(LogLevel::Error, error);
    ...
    fn handle_watch_error(&mut self, window_id: WindowId, tab_id: TabId, message: String) {
        ...
        if tab_id == WORKSPACE_TREE_WATCH_ID {
            state.app.push_log(LogLevel::Error, message);
            schedule_redraw(state);
            return;
        }
        if let Some(session) = state.app.document_by_legacy_tab_id_mut(tab_id) {
    ...
    fn open_workspace_in_window(&mut self, window_id: WindowId, path: PathBuf) {
        ...
        if let Err(error) = self.client.rebind_workspace(path.clone()) {
            state.app.push_log(LogLevel::Error, error);
            schedule_redraw(state);
            return;
        }
        ...
        if let Some(workspace_path) = state.app.workspace_path().map(PathBuf::from) {
            state.workspace_watch = self
                .client
                .subscribe_path(
                    &workspace_path,
                    build_workspace_changed_notifier(self.proxy.clone(), window_id),
                    build_workspace_error_notifier(self.proxy.clone(), window_id),
                )
                .ok();
        }
        state.window.set_title(&state.app.window_title());
        self.config.recent_workspaces = state.app.recent_workspaces().to_vec();
        if let Err(error) = self.client.save_config(&self.config) {
            state
                .app
                .push_log(LogLevel::Warning, format!("保存最近工作区失败: {error}"));
        }
        schedule_redraw(state);
    ...
    fn open_workspace_file(&mut self, window_id: WindowId, path: PathBuf) {
        ...
        if !path.is_file() {
            state
                .app
                .push_log(LogLevel::Warning, format!("文件不存在: {}", path.display()));
            schedule_redraw(state);
            return;
        }
        let extension = path
        ...
        let open_result = match kind {
            DocumentKind::Viewer => ViewerTab::open(
                ...
                self.config.openscad_path.clone(),
                self.proxy.clone(),
                window_id,
            )
            .map(StudioDocumentSession::Viewer),
            DocumentKind::Markdown => MarkdownTab::open(
                self.client.clone(),
                path.clone(),
                self.proxy.clone(),
                window_id,
            ...
            DocumentKind::Image => ImageTab::open(
                self.client.clone(),
                path.clone(),
                self.proxy.clone(),
                window_id,
        ...
        match open_result {
            ...
            Err(error) => state.app.push_log(LogLevel::Error, error),
        }
        schedule_redraw(state);
    }

    fn load_directory(&mut self, window_id: WindowId, path: PathBuf) {
        let Some(state) = self.windows.get_mut(&window_id) else {
            return;
        };
        match self.client.list_directory(&path) {
            Ok(entries) => state.app.set_file_tree_children(path, entries),
            Err(error) => state.app.push_log(LogLevel::Warning, error),
        }
        schedule_redraw(state);
    }
    ...
    fn save_app_config(&mut self, window_id: WindowId) {
        ...
        match self.client.save_config(&self.config) {
            Ok(()) => state.app.push_log(LogLevel::Info, "配置已保存"),
            Err(error) => state.app.push_log(LogLevel::Error, error),
        }
    }
...
fn attach_menu_handler(menu: &Option<PlatformMenu>, proxy: EventLoopProxy<UserEvent>) {
    if let Some(menu) = menu.as_ref() {
        menu.attach_event_handler(proxy, UserEvent::Menu);
    }
}
...
fn load_app_config(client: &DesktopProtocolClient) -> AppConfig {
...
fn build_workspace_changed_notifier(
    proxy: EventLoopProxy<UserEvent>,
    window_id: WindowId,
...
fn run_smoke_exit() -> Result<(), String> {
    let workspace = tempfile::Builder::new()
        .prefix("studio-app-smoke-")
        .tempdir()
        .map_err(|error| error.to_string())?;
    DesktopProtocolClient::run_smoke_check(workspace.path().to_path_buf())
}

fn build_workspace_error_notifier(
    proxy: EventLoopProxy<UserEvent>,
    window_id: WindowId,
