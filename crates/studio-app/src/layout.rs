...
use scad_ui::{theme, welcome::WelcomeAction};
...
pub fn show(
            ...
            .show(ctx, |ui| {
                ...
                if let Some(next) = left_panel::show(ui, app) {
                    action = Some(match next {
                        ...
                        LeftPanelAction::LoadDirectory(path) => LayoutAction::LoadDirectory(path),
    ...
    if let Some(next) = work_area::show(ctx, app, config, &mut viewer_outcome) {
        action = Some(match next {
            WelcomeAction::OpenFolder => LayoutAction::OpenFolder,
            WelcomeAction::OpenRecent(path) => LayoutAction::OpenRecent(path),
        });
    }
