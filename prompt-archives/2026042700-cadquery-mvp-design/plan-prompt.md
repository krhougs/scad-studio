...
11. **整体独立 review 与硬编码质疑**
    - 用户要求整体带 plan 和意图进行独立 review，直到 plan 完成验收。
    - 随后指出代码里不应写死“开孔”“槽”等自然语言几何意图。

12. **禁止硬编码，由 LLM 决定**
    - 用户要求继续检查代码里是否还有其它硬编码。
    - 用户进一步明确：不允许存在 move / replace 等硬编码判断，这些编辑意图应由 LLM 自行决定。

13. **删除 codegen fallback，改为 system prompt 设计**
